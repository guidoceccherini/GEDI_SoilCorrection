...
csv_filesMirror <- paste0("Data/EU_Grid25km_RH50_Mirror_", regions, ".csv")
combined_csvMirror <- lapply(csv_filesMirror, function(f) {
  df <- read.csv(f, stringsAsFactors = FALSE, colClasses = c(
    grid_id = "character",
    region = "character"
  ))
...
combined_csvMirror <- tibble(combined_csvMirror)
...
merged_data <- combined_shp %>%##st_drop_geometry() %>%
  ...
  left_join(combined_csvMirror, by = c("numeric_id", "region"))

merged_data <- merged_data |> drop_na(rh50_median.x )


merged_data <- merged_data |> mutate(DeltaGaussVsMirror = rh_non_ground50_median.x - rh_non_ground50_median.y)

# Create map with diverging palette
ggplot(merged_data) +
  geom_sf(aes(fill = DeltaGaussVsMirror), color = NA) +
  ...
  theme_minimal() +
  labs(title = "Difference between Non-Ground GAUSS and Mirror RH50 Median") +
  theme(
